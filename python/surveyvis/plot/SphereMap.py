...
from surveyvis.sphere import offset_sep_bear, horizon_to_eq, eq_to_horizon, rotate_cart
...
class SphereMap:
    ...
    def to_orth_zenith(self, hpx, hpy, hpz):
        ...
        npole_x2, npole_y2, npole_z2 = rotate_cart(
            1, 0, 0, self.lat + 90, npole_x1, npole_y1, npole_z1
        )
    ...
    def eq_to_horizon(self, ra, decl, degrees=True, cart=True):
        ...
        if cart:
            x, y = eq_to_horizon(
                ra, decl, self.lat, self.lst, degrees=degrees, cart=cart
            )
    ...
    def make_graticule_points(
        ...
        for decl in np.arange(min_decl, max_decl + decl_space, decl_space):
            ...
            x_hz, y_hz = self.eq_to_horizon(
                this_graticule["ra"], this_graticule["decl"]
            )
    ...
    def make_points(self, points_data):
        ...
        x_hz, y_hz = eq_to_horizon(
            points_df.ra.values,
            points_df.decl.values,
            self.lat,
            self.lst,
            degrees=True,
            cart=True,
        ...
        #invisible = zs > np.finfo(zs).resolution
        #xs[invisible] = np.nan
        #ys[invisible] = np.nan
        #zs[invisible] = np.nan
    ...
    def add_stars(
        self, points, plot, lat, sliders=None, star_kwargs={"color": "black"}
    ):
        plot.star(
            x="x_orth", y="y_orth", size="glyph_size", source=points, **star_kwargs
        )
